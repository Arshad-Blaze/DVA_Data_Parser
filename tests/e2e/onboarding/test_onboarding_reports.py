...
class TestOnboardingReports:
    ...
    def test_reports_heading(self, onb_page: Page, test_data: dict):
        page = onb_page
        self._navigate_to_reports(page, test_data)
        expect(page.get_by_text("Step 7: Reports")).to_be_visible()

    def test_execution_summary_displayed(self, onb_page: Page, test_data: dict):
        page = onb_page
        self._navigate_to_reports(page, test_data)
        expect(page.get_by_text("Execution Summary")).to_be_visible()

    def test_processing_history_displayed(self, onb_page: Page, test_data: dict):
        page = onb_page
        self._navigate_to_reports(page, test_data)
        expect(page.get_by_text("Processing History")).to_be_visible()

    def test_download_upc_summary_button(self, onb_page: Page, test_data: dict):
        page = onb_page
        self._navigate_to_reports(page, test_data)
        expect(page.get_by_role("button", name="Download UPC Summary")).to_be_visible()

    def test_start_over_from_reports(self, onb_page: Page, test_data: dict):
        page = onb_page
        self._navigate_to_reports(page, test_data)
