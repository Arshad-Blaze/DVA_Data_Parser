...
| `preview_flattened_multiline(...)` | File paths, record types | `pl.DataFrame` | Returns first N rows of flattened multiline data |
...
3. **Multiline path** — iterates `flatten_multiline_chunks` → optional column_names rename → same pipeline as fixed-width
...
- `_iter_chunks()` — dispatches to the correct chunk iterator based on file_type
...
| `is_multiline_record(path)` | Checks for 2+ different letter prefixes (H, D, etc.) or 5+ backslash-continuation lines in first 10 lines |
...
- **File review report** — all three file types
...
- **Detection** — all delimiter types, fixed-width, excel, multiline, header detection
...
2. Add a branch in `_iter_chunks()` in `_aggregators.py`
3. Add detection logic in `detection.py`
4. Add preview handling in `preview_raw()` in `_parsers.py`
