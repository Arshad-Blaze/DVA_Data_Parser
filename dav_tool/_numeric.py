...
def _convert_with_config(val: str, cfg: NumericParsingConfig) -> Optional[float]:
    """Utility to parse a single string value (non-vectorised)."""
    import re
    v = val.strip()
    if not v:
        return None
    if v.upper() in {p.upper() for p in cfg.null_patterns}:
        return None

    had_parens = False
    if cfg.negative_format == "parens" and v.startswith("(") and v.endswith(")"):
        had_parens = True
        v = v[1:-1]
    ...
    if cfg.thousands_separator:
        v = v.replace(cfg.thousands_separator, "")

    if cfg.decimal_separator != ".":
        v = v.replace(cfg.decimal_separator, ".")
